...
def convert_label(labels, dim):
    width = dim[0]
    height = dim[1]
    new_labels = []
    for i in range(labels.shape[0]):
        x1 = (labels[i][1] - labels[i][3] / 2) * width
        x2 = (labels[i][1] + labels[i][3] / 2) * width
        y1 = (labels[i][2] - labels[i][4] / 2) * height
        y2 = (labels[i][2] + labels[i][4] / 2) * height
        new_labels.append([x1, y1, x2, y2, 0, 1 , 0])
    return np.array(new_labels).round()
